...
class UserNotificationSettings(db.Model):
    ...
    # Уведомления о тестах (одобрение, редактирование модератором)
    uns_email_tests = db.Column(db.Boolean, default=True)
    # Сообщения от администратора (рассылки)
    uns_email_admin_messages = db.Column(db.Boolean, default=True)
    # Уведомления о смене имени или статуса администратором
...
def _create_notification(user_id, sender_id, text, link=None, category=None):
    ...
    Args:
        ...
        sender_id: ID отправителя (None для системных)
        text: Текст уведомления
        link: Ссылка (опционально)
        category: Категория для email-фильтрации:
                  'tests' — уведомления о тестах
                  'admin_messages' — сообщения от администратора
                  'account_changes' — смена имени/статуса
                  None — email не отправляется
    """
    ...
    if category:
        ...
        if user and user.email:
            ...
            send_email = False
            if settings is None:
                send_email = True  # по умолчанию всё включено
            ...
            if send_email:
                try:
                    ...
                    msg = Message(recipients=[user.email], body=body, subject=subject)
                    mail.send(msg)
...
def take_test(test_name):
    ...
    # Для каждого вопроса получаем ответы (перемешиваем для типов с вариантами)
...
@app.route('/profile/notification-settings', methods=['POST'])
@login_required
